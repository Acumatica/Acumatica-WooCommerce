...
{
    public class ImportSalesOrderAmountRefundFromWC : TestSalesOrderBase
    {
        ...
        public override void Execute()
        {
            using (TestExecution.CreateTestCaseGroup("Import a Sales Order for an existing Customer and Item"))
            {
                ...
                using (TestExecution.CreateTestStepGroup("Step 8 - Verify Refund Import Process Success in Acumatica."))
                {
                    VerifyRefundSyncSuccessInAcumatica(newOrder.Id.ToString(), false, Entities.Refund);
                }
        ...
        {
            refundData = DataService.GetOrderRefund(DataFilePath.ImportSalesOrderAmountRefundFromWC);
