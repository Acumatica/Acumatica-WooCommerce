...
namespace ACSC.Tests.ShopifyRest.Client.DataRepository.Inventory
{
    public class InventoryLocationRestDataProvider : RestDataProviderBase, IParentRestDataProvider<InventoryLocationData>
    {
        ...
        public List<InventoryLocationData> GetCurrentList(out string previousList, out string nextList, IFilter filter = null)
        {
            return GetCurrentList<InventoryLocationData, InventoryLocationsResponse>(out previousList, out nextList, filter);
        }

        public List<InventoryLocationData> GetAll(IFilter filter = null)
        {
